...
int Date::dateValue() const { return daysSince0001_1_1(); }

bool Date::operator==(const Date& RO) const {
  return daysSince0001_1_1() == RO.dateValue();
};
bool Date::operator!=(const Date& RO) const {
  return daysSince0001_1_1() != RO.dateValue();
};
...
ostream& Date::write(ostream& os = std::cout) const {
  if (bad()) {
    os << dateStatus();
  } else {
    os << m_year << "/" << setfill('0') << setw(2) << right << m_mon << "/"
       << setfill('0') << setw(2) << right << m_day << setw(0);
  }
  return os;
}

istream& Date::read(istream& is = std::cin) {
  char delimiter;
  errCode(NO_ERROR);
...
bool operator!(Date& RO) { return RO.errCode() != 0; };
